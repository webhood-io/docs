...
Webhood is designed primarily for cybersecurity professionals, but it can also be deployed to support other use cases. The primary use case for Webhood is to quickly check what's behind a URL by a person able recognize phishing from common indicators such as typosquatting, impersonating legitimate brands and other common indicators of a phishing site. Webhood brings you tools to analyze, but does not provide any automated analysis.

One of the key reasons why Webhood exists is to allow you to set up your own scanner instance and not have to worry about sharing data with third parties. Although sharing information about suspected phishing sites with the cybersecurity community is benefits the community, URL's gathered from end users may contain confidential data. For instance, an URL in an email may also contain information about the recipient that should not be shared anywhere outside your organization.
